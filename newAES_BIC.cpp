...
void AESwithFibonacci(){
    ...
    cout << fixed << totalTime/10000 << setprecision(6)<<endl;
}
...
double sbox_bic() {
    ...
    for (i = 0; i < 8; ++i) {
        ...
        for (j = 0; j < 8; ++j) {
            for (k = 0; k < 8; ++k) {
                if (j != k) {
                    aval_vector_j = (unsigned int*)calloc(two_power(8), sizeof(unsigned int));
                    aval_vector_k = (unsigned int*)calloc(two_power(8), sizeof(unsigned int));

                    // for each possible input
                    for (X = 0; X < two_power(8); ++X) {
                        ej = two_power(j);
                        ek = two_power(k);

                        dei = S_Box_Linear[X].to_ulong() ^ S_Box_Linear[X ^ ei].to_ulong();
                        dej = (dei & ej) >> j;
                        dek = (dei & ek) >> k;
                    ...
                    corr = fabs(correlation(aval_vector_j, aval_vector_k, two_power(8)));
                    // printf("corr[%d][%d] = %f\n", j, k, corr);
                    if (maxCorr < corr)
                        maxCorr = corr;
